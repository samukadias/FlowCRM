...
- [x] Catálogo de serviços (/catalogo, só ADMIN) com preço unitário padrão, e itens da PO (Planilha Orçamentária) em cada ESP: produto do catálogo, quantidade mensal, período contratual e valor unitário (negociável); analista responsável também anexa o relatório de projeto técnico. Itens travados para edição assim que a ESP é marcada como pronta.
- [x] Visibilidade dos itens da PO na proposta assinada (Contratos vê as ESPs e seus itens, somente leitura) e medição mensal por item no Faturamento: a atestação nasce com o valor planejado (soma quantidade × valor unitário dos itens) e uma medição por item pré-preenchida com o quantitativo contratado; o Faturamento ajusta cada medição ao que o cliente de fato consumiu no mês, recalculando o valor da atestação
